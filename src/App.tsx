...
import { OptimizedImage, getOptimizedImagePath, getImageSrcSet } from './components/OptimizedImage';
...
const PageLoader = () => (
  <div className="min-h-screen bg-salon-black pt-28" aria-hidden="true">
...
export default function App() {
  ...
  // Viewport monitor for mobile responsive optimizations
  useEffect(() => {
    const checkMobile = () => {
  ...
  // Consolidated API fetch — all endpoints in parallel
  const fetchDbData = async () => {
    try {
      const [settingsRes, svcRes, barRes] = await Promise.allSettled([
        fetch('/api/site-settings'),
        fetch('/api/services?branch_id=all'),
        fetch('/api/barbers'),
      ]);

      // Express /api/data (legacy fallback for settings/smtp/bookings)
      if (settingsRes.status === 'fulfilled' && settingsRes.value.ok) {
        try {
          const siteSettings = await settingsRes.value.json();
          if (siteSettings) setSettings({ ...siteSettings, heroBg: assetUrl(siteSettings.heroBg) });
  ...
  // Facebook Pixel SPA PageView Tracking
  useEffect(() => {
  ...
  // Facebook Pixel VIP Membership Lead Tracking
  useEffect(() => {
    if (vipFormSubmitted) {
      const fbq = (window as any).fbq;
      if (fbq) {
        fbq('track', 'Lead', {
          content_name: 'VIP Membership Request'
        });
      }
    }
  }, [vipFormSubmitted]);
  ...
  // Intersection Observer for maps lazy loading
  useEffect(() => {
  ...
  const handleVipSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (vipName.trim() && vipPhone.trim() && vipEmail.trim()) {
  ...
  if (currentPath.startsWith('/career/')) {
    const slug = currentPath.replace('/career/', '');
    return (
      <div className="bg-salon-black text-white selection:bg-gold-400 selection:text-salon-black min-h-screen relative font-sans leading-relaxed flex flex-col">
        <Header />
        <Suspense fallback={<PageLoader />}>
  ...
  if (currentPath === '/booking-confirmation') {
    return (
      <div className="bg-salon-black text-white selection:bg-gold-400 selection:text-salon-black min-h-screen relative font-sans leading-relaxed flex flex-col">
        <Header />
        <Suspense fallback={<PageLoader />}>
          <BookingConfirmationPage services={services} />
        </Suspense>
        {renderFooter()}
      </div>
    );
  ...
  return (
    <div className="landing-page bg-salon-black text-white selection:bg-gold-400 selection:text-salon-black min-h-screen relative font-sans leading-relaxed flex flex-col">
      ...
      >
        {/* Parallax Cover Image - optimized WebP with high priority */}
      ...
      <section id="services-section" className="py-20 md:py-24 bg-salon-black relative overflow-hidden border-b border-gold-400/10 text-center">
        <div
          ...
          style={isMobile ? { display: 'none' } : {
            backgroundImage: `image-set(url('${assetUrl('/assets/images/optimized/adonis_styling_chairs_1779270725139-800w.webp')}') 1x, url('${assetUrl('/assets/images/optimized/adonis_styling_chairs_1779270725139.webp')}') 2x)`,
      ...
      <LazySection id="amenities-section" className="relative bg-salon-black overflow-hidden" placeholderHeight="75vh">
        ...
        <div className="relative h-[75vh] sm:h-[85vh] md:h-[90vh] w-full overflow-hidden">
          <AnimatePresence mode="wait">
            ...
            >
              {/* Image with zoom */}
              <motion.img
                src={getOptimizedImagePath(AMENITIES[amenityIndex].img)}
                srcSet={getImageSrcSet(AMENITIES[amenityIndex].img)}
                ...
                decoding="async"
                fetchPriority="low"
                initial={isMobile ? false : { scale: 1.3 }}
                animate={isMobile ? false : { scale: 1 }}
                transition={isMobile ? undefined : { duration: 5, ease: 'easeOut' }}
      ...
      <LazySection id="team-section" className="py-20 md:py-24 bg-salon-black relative border-b border-gold-400/10" placeholderHeight="550px">
        <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-16">
          ...
          <Suspense fallback={<div className="h-40 flex items-center justify-center text-gray-500 font-mono text-xs">Loading Stylists...</div>}>
            <BarberCarousel
              barbers={barbers}
      ...
      <LazySection id="vip-section" className="py-20 md:py-24 bg-salon-black relative overflow-hidden border-b border-[#32BBED]/10" placeholderHeight="550px">
        <div
          ref={vipBgRef}
          className="absolute inset-y-[-6%] left-1/2 w-[100vw] bg-cover bg-center bg-no-repeat opacity-[0.38] pointer-events-none"
          style={isMobile ? { display: 'none' } : {
            backgroundImage: `url('${getOptimizedImagePath(assetUrl('/assets/images/vip.png'))}')`,
            backgroundSize: 'cover',
            backgroundPosition: 'center',
            backgroundRepeat: 'no-repeat',
            transform: 'translate3d(-50%, 0, 0) scale(1.08)',
      ...
      <LazySection id="booking-section" className="py-20 md:py-24 bg-salon-black relative border-b border-gold-400/10 scroll-mt-20" placeholderHeight="700px">
      ...
      <LazySection id="branch-section" className="py-20 md:py-24 bg-salon-gray relative border-b border-gold-400/10" placeholderHeight="600px">
        <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-16">
      ...
      {/* GALLERY SECTION */}
      <LazySection id="gallery-section" className="py-20 md:py-24 bg-salon-black relative border-b border-gold-400/10" placeholderHeight="900px">
        <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-16">
          <div className="text-center max-w-2xl mx-auto space-y-3">
            <span className="text-[10px] font-mono tracking-[0.3em] text-gold-400 uppercase block">
      ...
      <LazySection id="testimonials-section" className="py-20 md:py-24 bg-salon-gray relative border-b border-gold-400/10" placeholderHeight="350px">
        <div className="max-w-7xl mx-auto px-4 md:px-8 space-y-16">
          <div className="text-center max-w-2xl mx-auto space-y-3">
            <span className="text-[10px] font-mono tracking-[0.3em] text-gold-400 uppercase block">
          ...
          <Suspense fallback={<div className="h-40 flex items-center justify-center text-gray-500 font-mono text-xs">Loading Reviews...</div>}>
      ...
      {/* CONTACT SECTION */}
      <LazySection id="contact-section" className="py-20 md:py-24 bg-salon-gray relative border-b border-gold-400/10" placeholderHeight="600px">
                  ...
                  <form onSubmit={handleVipSubmit} className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-salon-gray text-white text-xs border border-white/10 px-4 py-3 focus:outline-none focus:border-gold-400 transition-colors disabled:opacity-50"
                      />
                    </div>
                    ...
                    <button
                      type="submit"
                      disabled={vipSubmitting}
                      className="w-full py-3 bg-gradient-to-r from-gold-500 to-gold-400 text-salon-black font-serif text-xs font-bold uppercase tracking-widest transition-all cursor-pointer disabled:opacity-50"
                    >
                      {vipSubmitting ? 'Processing Invitation...' : 'Process Invitation Request'}
                    </button>
                  </form>
                ) : (
                  <motion.div
