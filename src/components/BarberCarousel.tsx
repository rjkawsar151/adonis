...
export const BarberCarousel: React.FC<BarberCarouselProps> = ({ barbers, onBookBarber }) => {
  ...
  return (
    <div 
      className="relative w-full max-w-6xl mx-auto px-4 md:px-14 select-none group box-sizing-border-box"
      style={{
        // Set CSS variables for fluid, accurate slider math
        ['--items-per-page' as any]: itemsPerPage,
        ['--card-gap' as any]: `${gap}px`,
        ['--card-width' as any]: `calc((100% - (var(--items-per-page) - 1) * var(--card-gap)) / var(--items-per-page))`
      }}
    >
      {/* Slider viewport */}
      <div className="overflow-hidden w-full py-4 rounded-none">
        <div
            ...
            >
              <div>
                ...
                <div className="aspect-[4/5] w-full overflow-hidden relative">
                  <OptimizedImage
                    ...
                    alt={barber.name}
                    className="w-full h-full object-cover object-[50%_15%] brightness-95 hover:grayscale hover:brightness-90 transition-all duration-500"
                    width={1024}
                    height={1536}
                ...
                <div className="p-5 space-y-2">
                  ...
                  <h4 className="font-serif text-sm uppercase tracking-wider text-white group-hover:text-gold-400 transition-colors truncate">
                    {barber.name}
                  </h4>
                  <p className="text-[10px] font-mono tracking-widest text-gray-400 uppercase truncate">
                    {barber.specialty}
                  </p>
                  <p className="text-[10px] text-gray-500 block truncate">
                    Grooming Specialist • {barber.experienceYears}+ Years Experience
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 pt-0">
                <button
                  ...
                  className="w-full py-2.5 bg-[#32BBED] hover:bg-[#b08d3c] text-black hover:text-black font-serif text-[10px] font-bold uppercase tracking-widest transition-all duration-300 cursor-pointer text-center whitespace-nowrap overflow-hidden"
                >
                  Book Session
                </button>
              </div>
      ...
      {barbers.length > itemsPerPage && (
        <>
          <button
            onClick={handlePrev}
            className="absolute left-2 md:left-4 top-1/2 -translate-y-1/2 h-10 w-10 bg-salon-black/90 border border-white/10 hover:border-gold-400 hover:text-gold-400 flex items-center justify-center text-white transition-all cursor-pointer rounded-none z-10 shadow-lg"
          ...
          <button
            onClick={handleNext}
            className="absolute right-2 md:right-4 top-1/2 -translate-y-1/2 h-10 w-10 bg-salon-black/90 border border-white/10 hover:border-gold-400 hover:text-gold-400 flex items-center justify-center text-white transition-all cursor-pointer rounded-none z-10 shadow-lg"
