...
export const Header: React.FC = () => {
  ...
  useEffect(() => {
    // Check if any active jobs are posted (deferred to not block critical load)
    const timer = setTimeout(() => {
      fetch('/api/careers')
        .then(res => res.json())
        .then(data => {
          if (data && data.jobs && data.jobs.total > 0) {
            setHasCareers(true);
          }
        })
          ...
          <nav className="hidden lg:flex items-center space-x-7">
            {navItems.map((item) => (
              <a
                ...
                className={`text-xs font-sans font-bold uppercase tracking-[0.15em] transition-all duration-300 relative after:content-[''] after:absolute after:-bottom-1 after:left-0 after:right-0 after:h-[1.5px] after:bg-[#32BBED] after:transition-transform after:duration-300 ${currentPath === item.href && !item.hash
                    ? 'text-[#32BBED] after:scale-x-100'
                    : 'text-white hover:text-[#32BBED] after:scale-x-0 hover:after:scale-x-100'
                  }`}
              >
                {item.label}
              </a>
              ...
              <div className="px-4 py-6 space-y-4 flex flex-col">
                {navItems.map((item) => (
                  <a
                    ...
                    href={item.hash ? `${item.href}${item.hash}` : item.href}
                    onClick={(e) => handleNavClick(e, item.href, item.hash)}
                    className={`text-xs font-sans font-bold uppercase tracking-widest py-1 transition-colors ${currentPath === item.href && !item.hash ? 'text-[#32BBED]' : 'text-white hover:text-[#32BBED]'
                      }`}
                  >
                    {item.label}
                  </a>
