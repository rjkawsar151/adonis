...
export const ServicesPage: React.FC<ServicesPageProps> = ({ initialBranch = 'gulshan', onBookService, priceList }) => {
  ...
  const activePriceList: PriceGroup[] = useMemo(() => {
    ...
    if (!priceListSource) return [];
    const list = selectedBranch === 'bashundhara' ? priceListSource.bashundhara : priceListSource.gulshan;
    return list || [];
  }, [selectedBranch, priceList, internalPriceList]);
