...
import { Booking, Service, PriceGroup, PriceListItem, Barber } from '../types';
...
export const BookingForm: React.FC<BookingFormProps> = ({
  ...
  // Internal price list state if not provided externally
  const [internalPriceList, setInternalPriceList] = useState<{ gulshan: PriceGroup[]; bashundhara: PriceGroup[] }>({
    gulshan: [],
    bashundhara: [],
  });
  ...
  useEffect(() => {
    ...
    const loadPriceList = async () => {
      try {
        const [plGulRes, plBashRes] = await Promise.allSettled([
          fetch('/api/price-list/gulshan'),
          fetch('/api/price-list/bashundhara'),
        ]);
        const gulshan = plGulRes.status === 'fulfilled' && plGulRes.value.ok ? await plGulRes.value.json() : [];
        const bashundhara = plBashRes.status === 'fulfilled' && plBashRes.value.ok ? await plBashRes.value.json() : [];
        setInternalPriceList({
          gulshan: Array.isArray(gulshan) ? gulshan : [],
          bashundhara: Array.isArray(bashundhara) ? bashundhara : [],
        });
      } catch (err) {
        // Fallback silently
      }
  ...
  const priceListSource = priceList || internalPriceList;
  const activePriceList = selectedBranch === 'bashundhara' && priceListSource.bashundhara && priceListSource.bashundhara.length > 0
    ? priceListSource.bashundhara
    : priceListSource.gulshan || [];
        ...
        {!successBooking ? (
                <form onSubmit={handleSubmit} className="space-y-8">
                  ...
                  <div>
                    ...
                    {selectedBranch === 'bashundhara' && (!priceListSource.bashundhara || priceListSource.bashundhara.length === 0) ? (
                  ...
                  {/* Step 2.5: Barber Selection */}
                  <div className="space-y-2">
                    <label className="block text-xs font-serif uppercase tracking-widest text-gold-400 mb-2">
                      Step 2.5: Select Grooming Expert (Optional)
                    </label>
                    <div className="relative">
                      <select
                        value={selectedBarber}
                        onChange={e => setSelectedBarber(e.target.value)}
                        className="w-full bg-[#111] hover:bg-salon-gray text-white text-xs border border-white/10 px-4 py-3 focus:outline-none focus:border-gold-400 transition-colors uppercase tracking-widest rounded-none"
                      >
                        <option value="">Any Stylist (Recommended)</option>
                        {barbers && barbers.map(barber => (
                          <option key={barber.id} value={barber.id} className="bg-salon-black">
                            {barber.name} - {barber.specialty || 'Master Stylist'}
                          </option>
                        ))}
                      </select>
                    </div>
