...
@section('page_title', 'Add Stylist')

@section('admin_content')
<form action="{{ url('/admin/barbers') }}" method="POST" enctype="multipart/form-data" class="max-w-2xl bg-[#111827] border border-gray-800 p-8 space-y-6">
    @csrf

    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">Full Name</label>
            <input type="text" name="name" value="{{ old('name') }}" required class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">Specialty</label>
            <input type="text" name="specialty" value="{{ old('specialty') }}" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">Experience (Years)</label>
            <input type="number" name="experienceYears" value="{{ old('experienceYears', 0) }}" min="0" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">Rating (0–5)</label>
            <input type="number" step="0.1" name="rating" value="{{ old('rating', 5.0) }}" min="0" max="5" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div class="md:col-span-2 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="portrait_url" value="{{ old('portrait_url') }}" placeholder="e.g. /assets/images/dulal_chandra.png" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
            </div>
        ...
        <div class="md:col-span-2">
            ...
            <textarea name="bio" rows="4" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C] resize-none">{{ old('bio') }}</textarea>
        </div>
    </div>

    <div class="flex gap-4 pt-2">
        <button type="submit" class="px-6 py-2.5 bg-[#C9A84C] hover:bg-[#b8973f] text-black text-xs font-bold uppercase tracking-wider transition-colors">Save Stylist</button>
