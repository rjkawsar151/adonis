...
@section('title', 'Edit Price List Item')
...
<form action="{{ url('/admin/price-list/' . $item->id) }}" method="POST" class="max-w-2xl">
    @csrf
    @method('PUT')
    ...
    <div class="bg-[#111827] border border-gray-800 p-8 space-y-6">
        ...
        <div>
            ...
            <input type="text" name="name" value="{{ old('name', $item->name) }}" required class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors">
        </div>

        <div>
            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Description</label>
            <textarea name="description" rows="2" class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors">{{ old('description', $item->description) }}</textarea>
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-3 gap-6">
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Price *</label>
                <input type="text" name="price" value="{{ old('price', $item->price) }}" required class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors">
            </div>
            <div>
                ...
                <select name="branch_id" class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors">
                    <option value="all" {{ old('branch_id', $item->branch_id) === 'all' ? 'selected' : '' }}>All Branches</option>
                    <option value="gulshan" {{ old('branch_id', $item->branch_id) === 'gulshan' ? 'selected' : '' }}>Gulshan Only</option>
                    <option value="bashundhara" {{ old('branch_id', $item->branch_id) === 'bashundhara' ? 'selected' : '' }}>Bashundhara Only</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Sort Order *</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $item->sort_order) }}" required class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors">
            </div>
        </div>
    </div>
