...
<form action="{{ url('/admin/blogs/' . $blog->id) }}" method="POST" enctype="multipart/form-data" class="max-w-3xl bg-[#111827] border border-gray-800 p-8 space-y-6">
    @csrf
    @method('PUT')

    <div class="flex items-center justify-between border-b border-gray-800 pb-4">
        <h4 class="font-bold text-sm uppercase tracking-wider text-[#C9A84C]">Edit Post</h4>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $blog->title) }}" required class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">URL Slug</label>
            <input type="text" name="slug" value="{{ old('slug', $blog->slug) }}" required class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div class="md:col-span-2">
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">Short Excerpt</label>
            <textarea name="excerpt" rows="2" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C] resize-none">{{ old('excerpt', $blog->excerpt) }}</textarea>
        </div>
        <div class="md:col-span-2">
            ...
            @if($blog->coverImage)
                <div class="mt-2 w-28 h-20 border border-gray-700 overflow-hidden bg-black" id="current-cover">
                    <img src="{{ asset($blog->coverImage) }}" alt="Current Cover" class="h-full w-full object-cover">
                </div>
            @endif
        ...
        <div class="md:col-span-2">
            ...
            <div class="border border-gray-700 bg-[#0c0f15] overflow-hidden">
                ...
                >{!! $blog->contentHtml !!}</div>
                <textarea name="contentHtml" id="contentHtml" class="hidden">{{ old('contentHtml', $blog->contentHtml) }}</textarea>
            </div>
        </div>
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">SEO Title</label>
            <input type="text" name="seoTitle" value="{{ old('seoTitle', $blog->seoTitle) }}" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C]">
        </div>
        <div>
            <label class="block text-[10px] font-mono uppercase tracking-widest text-gray-500 mb-1.5">SEO Description</label>
            <textarea name="seoDescription" rows="2" class="w-full bg-[#0c0f15] text-white text-sm border border-gray-700 px-4 py-2.5 focus:outline-none focus:border-[#C9A84C] resize-none">{{ old('seoDescription', $blog->seoDescription) }}</textarea>
        </div>
    </div>

    <div class="flex gap-4 pt-2">
        <button type="submit" class="px-6 py-2.5 bg-[#C9A84C] hover:bg-[#b8973f] text-black text-xs font-bold uppercase tracking-wider transition-colors">Update Blog Post</button>
