...
<form action="{{ url('/admin/services') }}" method="POST" class="space-y-8">
    @csrf
    ...
    <div class="bg-[#111827] border border-gray-800 p-8 space-y-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="title" id="title" value="{{ old('title') }}" required class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors" placeholder="e.g. Precision Haircut">
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">URL Slug *</label>
                <input type="text" name="slug" id="slug" value="{{ old('slug') }}" required class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors font-mono" placeholder="e.g. precision-haircut">
            </div>
        </div>

        <div>
            <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Description</label>
            <textarea name="short_description" rows="3" class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors" placeholder="Brief description of the service...">{{ old('short_description') }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div>
                ...
                <select name="branch_id" class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors">
                    <option value="gulshan" {{ old('branch_id', request('branch', '')) === 'gulshan' ? 'selected' : '' }}>Gulshan Premium Lounge</option>
                    <option value="bashundhara" {{ old('branch_id', request('branch', '')) === 'bashundhara' ? 'selected' : '' }}>Bashundhara Premium Lounge</option>
                </select>
            </div>
            <div>
                <label class="block text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">Price (৳)</label>
                <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="w-full px-4 py-3 bg-[#1a1f2e] border border-gray-700 text-sm text-white focus:outline-none focus:border-[#C9A84C] transition-colors" placeholder="e.g. 700">
            </div>
            <div>
