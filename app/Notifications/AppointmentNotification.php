...
class AppointmentNotification extends Notification
{
    ...
    public function toMail($notifiable)
    {
        ...
        } elseif ($this->recipientType === 'status_update') {
            $status = ucfirst($this->appointment->status);
            return (new MailMessage)
                ->subject("Adonis Appointment Status Update: {$status}")
                ->greeting("Dear {$this->appointment->name},")
                ->line("The status of your appointment reservation has been updated.")
                ->line("Booking Reference: #{$this->appointment->id}")
                ->line("New Status: **{$status}**")
                ->line("Preferred Date: {$this->appointment->preferred_date}")
                ->line("Preferred Time: {$this->appointment->preferred_time}")
                ->line("If you have any questions, please contact the lounge directly.")
                ->line("Regards,")
                ->line("Adonis Men's Grooming");
        } else {
            return (new MailMessage)
